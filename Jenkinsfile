    ...
    options { skipDefaultCheckout() }
      stages {
		...
		stage('Approve the PR request') {
			...
			steps {
				echo "Approve"
				sh "curl --user tonysandeep:870184e9322f1f6eecfcb3854eae5b07ae168aab --data '{\"body\":\"This PR build is success from ${BUILD_TAG}\",\"event\":\"APPROVE\"}' --header Content-Type:application/json  --request POST https://api.github.com/repos/sangeethak92/CRUD-Release/pulls/${PR_NUMBER}/reviews"
			}
			post {
				success{
					script { 
